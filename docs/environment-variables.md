...
| `GJC_SESSION_CONTEXT_BUDGET_BYTES` | Overrides the synchronous session-context materialization budget in bytes (default `536870912` = 512 MiB). A positive integer raises/lowers the ceiling that `SessionContextTooLargeError` enforces; set it above your measured session size to suppress the preflight. |
