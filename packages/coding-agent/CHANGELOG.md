...
- Enabled explicit-path resumes now validate descriptor-bound commit metadata, base and tail hashes, terminal proof, and hot-record digests before loading only the authenticated post-compaction tail. Fresh-process gates enforce ≤64 MiB RSS for 120k-record compaction cycles, direct and captured forks, and one-million-record first build plus exact lazy reopen. Disposable `.spill.*` state is colocated with the transcript artifact root, and managed candidate scope inspection is streaming; retained managed startup remains on the eager compatibility path pending retained-authority range support.
- Enabled canary sessions now fail closed to shadow/eager behavior after two consecutive sidecar build failures or any failed adoption of an existing published sidecar (a 100% per-session reload-failure rate, above the fixed 0.1 threshold), expose the live auto-disable reason through `/session`, and clear the session-local latch through the existing `off` rollback mode. RSS coverage now includes the explicit 60k checkpoint and a direct 120k-to-one-million first-open slope assertion.
- Live `enabled` → `shadow`/`off` rollback no longer eagerly reloads retired history: the current process keeps bounded lazy reads from its authenticated disposable sidecar, while the next `off`-mode process start ignores derived state and restores the eager compatibility path.
