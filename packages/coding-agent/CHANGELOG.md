...
- Conversation store file locks now release idempotently and self-heal same-process leaks: a release that fails to close the lock handle no longer skips the unlink, the unlink is ownership-checked so it can only remove the exact lock this holder wrote, and a waiter that finds a well-formed lock recorded under this process's own pid with no live holder removes it instead of timing out. The 1000ms lock timeout is unchanged and live cross-process locks are never deleted.
- Conversation store file locks now release idempotently and self-heal same-process leaks: a release that fails to close the lock handle no longer skips the unlink, the unlink is ownership-checked so it can only remove the exact lock this holder wrote, and a waiter that finds a well-formed lock recorded under this process's own pid with no live holder reclaims it through an ownership-tracked mutex instead of timing out. Concurrent healers cannot delete a successor lock; an unlink failure follows the normal timeout path rather than spinning. The 1000ms lock timeout is unchanged and live cross-process locks are never deleted.
- Conversation store file locks now release idempotently and self-heal same-process leaks: a release that fails to close the lock handle no longer skips the unlink, the unlink is ownership-checked so it can only remove the exact lock this holder wrote, and a waiter that finds a well-formed lock recorded under this process's own pid with no live holder reclaims it through an ownership-tracked mutex instead of timing out. Concurrent healers cannot delete a successor lock; stale unpublished reclaim markers are recovered, and an unlink failure follows the normal timeout path rather than spinning. The 1000ms lock timeout is unchanged and live cross-process locks are never deleted.
