...
- Conversation-store lock cleanup now tolerates Bun's `EBADF` only when a successfully used lock handle is already closed, then still removes the owned lock file; all other cleanup errors remain fatal.
