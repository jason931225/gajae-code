...
- Enabled explicit-path resumes now validate descriptor-bound commit metadata, base and tail hashes, terminal proof, and hot-record digests before loading only the authenticated post-compaction tail. Opt-in 120k-record process gates enforce a ≤64 MiB fresh-reopen RSS delta and ≤64 MiB post-compaction RSS range; managed resumes remain on the eager compatibility path pending retained-authority range support.
