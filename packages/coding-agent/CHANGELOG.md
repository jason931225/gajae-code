...
### Fixed
- Telegram image delivery now sends JPEG and PNG through `sendPhoto`, falls back to named `sendDocument` uploads for WebP, GIF, SVG, AVIF, BMP, TIFF, HEIC, HEIF, and unknown image types, and preserves MIME types for files sent with `telegram_send`.
