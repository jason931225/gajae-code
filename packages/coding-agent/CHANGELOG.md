...
- Speech-to-text setup now reports platform-correct recorder installation commands and prints the complete enable/record/transcribe flow after a successful check or install. The operator guide now covers the Alt+H toggle, a Ctrl+P command-palette fallback, Ghostty's `macos-option-as-alt` requirement, macOS microphone permission, model behavior, troubleshooting, and key remapping.
