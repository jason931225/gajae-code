...
- Enabled canary sessions now fail closed to shadow/eager behavior after two consecutive sidecar build failures, expose the live auto-disable reason through `/session`, and clear the session-local latch through the existing `off` rollback mode. RSS coverage now includes the explicit 60k checkpoint and a direct 120k-to-one-million first-open slope assertion.
