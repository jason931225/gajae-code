...
- Managed session tree capture no longer aborts resume when a legacy `local://` (or other foreign-written) descendant under the managed session path is group/other-readable. `captureTree` re-secures owner-only modes once and retries on `mode_mismatch`, matching the managed-scope prepare self-heal, so managed legacy-local migration and task fallback authority restore succeed without relaxing the owner-only contract (#3950).
