...
- Same-directory managed session opens now retain directory and transcript identity from admission through return: existing bounded resumes reject deletion or replacement without persistence, while initially missing targets are created with retained no-replace publication, post-publication identity validation, and sidecar initialization only after the transcript is accepted.
