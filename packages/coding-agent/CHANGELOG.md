...
- Provider stream timeouts now retry on single-model sessions under a bare default configuration. Previously the legacy (non-managed-fallback) retry path was gated behind explicitly-set `retry.*` settings, so a "stream timed out while waiting for the first event" / "stream stalled while waiting for the next event" failure surfaced immediately and left the agent idle. Clearly-transient stream timeouts now retry even when no `retry.*` keys are configured, while `retry.enabled=false` and fail-closed handling of generic unknown errors are preserved.
