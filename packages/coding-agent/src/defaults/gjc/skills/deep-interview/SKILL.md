...
For ordinary diagnosis, run `gjc deep-interview sanity-check --json` first, then use the issue's selector with `inspect` to read only the affected summary, topology, pending shell, round, facts, triggers, or floor. Every typed deep-interview command and `draft create` defaults its session from `GJC_SESSION_ID`; omit `--session-id` inside the active session rather than probing or printing environment variables. An explicit `--session-id` remains available for intentional cross-session repair. Repair a healthy, repairable local issue with its matching typed operation (`initialize-context`, `confirm-topology`, `record-answer`, or `apply-round-result`) and the current revision. Do not use grep/sed edits, direct `.gjc/` edits, generic envelope replacement, or `--force` as a repair mechanism; use generic `gjc state` only for the compatibility clear/handoff paths documented here.
During deep-interview, Bash invocations are blocked only when the runtime extracts an actual filesystem mutation target. Read-only wrappers around `gjc deep-interview` and shell metacharacters inside inert single-quoted CLI values are allowed; do not rewrite a direct typed command merely to appease the phase guard.
