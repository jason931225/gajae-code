...
class ManagerFakeTransport implements MCPTransport {
	...
	async request<T = unknown>(method: string, _params?: Record<string, unknown>, _options?: MCPRequestOptions): Promise<T> {
...
class SharedToolTransport extends ManagerFakeTransport {
	constructor(readonly generation = 1, private failFirstCall = false) {
		super();
	}
	callCount = 0;

	async request<T = unknown>(method: string, params?: Record<string, unknown>, options?: MCPRequestOptions): Promise<T> {
		if (method === "tools/list") return { tools: [{ name: `shared-${this.generation}`, inputSchema: { type: "object" } }] } as T;
...
class SharedPromptTransport extends ManagerFakeTransport {
	...
	override async request<T = unknown>(method: string, params?: Record<string, unknown>, options?: MCPRequestOptions): Promise<T> {
		...
		if (method === "prompts/get") return { description: "greeting", messages: [{ role: "user", content: { type: "text", text: "hello" } }] } as T;
		return super.request<T>(method, params, options);
	}
}

class MultiServerToolTransport extends ManagerFakeTransport {
	constructor(readonly serverName: string, readonly generation: number) {
		super();
	}

	override async request<T = unknown>(method: string, params?: Record<string, unknown>, options?: MCPRequestOptions): Promise<T> {
		if (method === "tools/list") return { tools: [{ name: `${this.serverName}-${this.generation}`, inputSchema: { type: "object" } }] } as T;
		if (method === "tools/call") return { content: [{ type: "text", text: `${this.serverName}-ok-${this.generation}` }] } as T;
		return super.request<T>(method, params, options);
	}
}
...
test("disconnectAll aborts a hanging prepared lease open", async () => {
	...
	const operation = manager.withPreparedLease("hanging", { type: "stdio", command: "fake-mcp" }, async () => undefined);
	await Bun.sleep(0);
	const shutdown = manager.disconnectAll();
...
test("manager disconnectAll aggregates a rejecting retired HTTP-style close", async () => {
	...
	expect((rejection as AggregateError).errors).toEqual(
		expect.arrayContaining([
			expect.objectContaining({ name: "MCPPoolLeaseReleaseError", message: expect.stringContaining("retired close failed") }),
	]),
	);
	expect(transports[0]?.closeCount).toBe(1);
	expect(transports[1]?.closeCount).toBe(1);
	...
	test("per-session manager facades remain isolated while shared tools-only facades pool one child", async () => {
		...
		const pool = new MCPConnectionPool({
			...
			connect: async (name, config) => {
				...
				const transport = new ManagerFakeTransport();
				transports.push(transport);
				return { name, config, transport, serverInfo: { name: "fake", version: "1" }, capabilities: { tools: {} } } satisfies MCPServerConnection;
			},
		});
...
test("shared prompt execution stays lease-bound while one manager releases", async () => {
	...
	try {
		...
		for (let attempt = 0; attempt < 100 && !transport?.requests.includes("resources/list"); attempt += 1) await Bun.sleep(0);
		first.setNotificationsEnabled(true);
		for (let attempt = 0; attempt < 100 && !transport?.requests.includes("resources/subscribe"); attempt += 1) await Bun.sleep(0);
...
test("shared replacement is deferred across an unrelated reconnecting server", async () => {
	...
	const pool = new MCPConnectionPool({
		...
		connect: async (name, config) => {
			...
			transports.push(transport);
			return { name, config, transport, serverInfo: { name: "multi", version: String(generation) }, capabilities: { tools: {} } } satisfies MCPServerConnection;
		},
	});
	...
	try {
		...
		expect(manager.getTools().map(tool => tool.mcpToolName).sort()).toEqual(["a-1", "b-1"]);
		...
		await reconnectA;
		for (let attempt = 0; attempt < 100 && manager.getTools().find(tool => tool.mcpServerName === "b")?.mcpToolName !== "b-2"; attempt += 1) await Bun.sleep(10);
		expect(manager.getTools().map(tool => tool.mcpToolName).sort()).toEqual(["a-2", "b-2"]);
...
test("shared replacement rebind reconciles a lease acquired before a second rotation", async () => {
	...
	const pool = new MCPConnectionPool({
		...
		connect: async (name, config) => {
			...
			const transport = new MultiServerToolTransport(name, generation);
			transports.push(transport);
			return { name, config, transport, serverInfo: { name: "generation", version: String(generation) }, capabilities: { tools: {} } } satisfies MCPServerConnection;
		},
	});
	...
	try {
		...
		for (let attempt = 0; attempt < 100 && peer.getTools().find(tool => tool.mcpServerName === "remote")?.mcpToolName !== "remote-3"; attempt += 1) await Bun.sleep(10);
...
test("shared initial join reconciles a lease acquired before owner rotation", async () => {
	...
	const pool = new MCPConnectionPool({
		...
		connect: async (name, config) => {
			...
			const transport = new MultiServerToolTransport(name, generation);
			transports.push(transport);
			return { name, config, transport, serverInfo: { name: "initial-join", version: String(generation) }, capabilities: { tools: {} } } satisfies MCPServerConnection;
		},
	});
...
test("initial join hook rejection releases the acquired lease", async () => {
	...
	const pool = new MCPConnectionPool({
		...
		connect: async (name, config) => {
			...
			transports.push(transport);
			return { name, config, transport, serverInfo: { name: "hook-rejection", version: String(opens) }, capabilities: { tools: {} } } satisfies MCPServerConnection;
		},
	});
	const manager = new MCPManager(".", null, {
		pool,
...
test("initial join retry failure clears pending state for a subsequent connect", async () => {
	...
	const pool = new MCPConnectionPool({
		...
		connect: async (name, config) => {
			...
			const transport = new MultiServerToolTransport(name, generation);
			transports.push(transport);
			return { name, config, transport, serverInfo: { name: "retry-failure", version: String(generation) }, capabilities: { tools: {} } } satisfies MCPServerConnection;
		},
	});
...
test("shared replacement is requeued when reconnect starts during acquisition", async () => {
	...
	const pool = new MCPConnectionPool({
		...
		connect: async (name, config) => {
			...
			return { name, config, transport, serverInfo: { name: "inverse", version: String(generation) }, capabilities: { tools: {} } } satisfies MCPServerConnection;
		},
	});
	...
	try {
		...
		allowAReconnect.resolve();
		await reconnectA;
		for (let attempt = 0; attempt < 100 && manager.getTools().find(tool => tool.mcpServerName === "b")?.mcpToolName !== "b-4"; attempt += 1) await Bun.sleep(10);
		expect(manager.getTools().map(tool => tool.mcpToolName).sort()).toEqual(["a-2", "b-4"]);
		expect(owner.getTools().map(tool => tool.mcpToolName)).toEqual(["b-2"]);
		await manager.disconnectAll();
		await owner.disconnectAll();
		expect(pool.size).toBe(0);
...
test("queued shared replacement is dropped when teardown begins", async () => {
	...
	const pool = new MCPConnectionPool({
		...
		connect: async (name, config) => {
			...
			const transport = new MultiServerToolTransport(name, generation);
			transports.push(transport);
			return { name, config, transport, serverInfo: { name: "drop", version: String(generation) }, capabilities: { tools: {} } } satisfies MCPServerConnection;
		},
	});
...
test("shared replacement rebind is fenced when peer disconnects during old-lease teardown", async () => {
	...
	try {
		...
		for (let attempt = 0; attempt < 100 && !oldTransport.requests.includes("resources/list"); attempt += 1) await Bun.sleep(0);
		peer.setNotificationsEnabled(true);
		for (let attempt = 0; attempt < 100 && !oldTransport.requests.includes("resources/subscribe"); attempt += 1) await Bun.sleep(0);
...
test("shared noReplay request failure coordinates one replacement without resending the call", async () => {
	...
	const pool = new MCPConnectionPool({
		connect: async (name, config) => {
			...
			transports.push(transport);
			return { name, config, transport, serverInfo: { name: "recovery", version: "1" }, capabilities: { tools: {} } } satisfies MCPServerConnection;
		},
	});
	...
	try {
		...
		for (let attempt = 0; attempt < 100 && second.getTools()[0]?.mcpToolName !== "shared-2"; attempt += 1) await Bun.sleep(10);
		expect(opens).toBe(2);
...
test("shared transport crash has one restart owner across manager facades", async () => {
	...
	test("shared HTTP leases use one MCP session and one callback stream while releasing independently", async () => {
					...
					return Response.json(
						{
							jsonrpc: "2.0",
							id: message.id,
							result: { protocolVersion: "2025-03-26", capabilities: { tools: {} }, serverInfo: { name: "shared-http", version: "1" } },
						},
		...
		const config: MCPServerConfig = { type: "http", url: `${server.url.href}mcp?tenant=one`, sharing: "shared", timeout: 1_000 };
		try {
			await expect(first.connectServers({ remote: config }, {})).resolves.toMatchObject({ connectedServers: ["remote"] });
			await expect(second.connectServers({ remote: config }, {})).resolves.toMatchObject({ connectedServers: ["remote"] });
...
test("S7-style HTTP stub keeps distinct paths and queries on one host in separate entries", async () => {
				...
				return Response.json(
					{
						jsonrpc: "2.0",
						id: message.id,
						result: { protocolVersion: "2025-03-26", capabilities: { tools: {} }, serverInfo: { name: "s7", version: "1" } },
					},
	...
	const left = await pool.acquire("remote", { type: "http", url: `${base}alpha?tenant=one&tenant=two`, sharing: "shared" }, { sharingMode: "shared" });
	const right = await pool.acquire("remote", { type: "http", url: `${base}beta?tenant=one&tenant=two`, sharing: "shared" }, { sharingMode: "shared" });
	try {
		expect(pool.getHealth()).toHaveLength(2);
		expect(new Set(initializePaths)).toEqual(new Set(["/alpha?tenant=one&tenant=two", "/beta?tenant=one&tenant=two"]));
	} finally {
		await left.release();
		await right.release();
		server.stop(true);
