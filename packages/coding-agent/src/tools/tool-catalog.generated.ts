...
export const TOOL_CATALOG: Readonly<Record<string, ToolCatalogEntry>> = {
						...
						properties: {
							...
							workflowGate: {
								...
								properties: {
									stage: {
										type: "string",
										enum: ["deep-interview", "ralplan", "ultragoal"],
										description: "workflow gate stage",
									},
									kind: {
										type: "string",
										enum: ["question", "approval", "execution"],
		...
		parameters: {
			...
			properties: {
				...
				run: {
					...
					minLength: 1,
					description:
						"Shell command evaluated at each revision. Exit 0 = good, 125 = skip, any other non-zero = bad.",
				},
				invert: {
					default: false,
					description:
						"Find the commit that FIXED the behavior instead of the one that broke it (exit 0 is treated as bad).",
					type: "boolean",
				},
				maxSteps: {
					default: 40,
			...
			properties: {
				calculations: {
					...
					items: {
						...
						required: ["expression", "prefix", "suffix"],
						additionalProperties: false,
					},
			...
			anyOf: [
				{
					oneOf: [
						...
						},
						{
							...
							required: ["action", "x", "y", "scroll_x", "scroll_y"],
							additionalProperties: false,
						},
						{
							type: "object",
		...
		parameters: {
			...
			properties: {
					...
					items: {
						...
						properties: {
							...
							repositoryBinding: {
								...
								required: ["schema", "worktreeRoot", "commonDir"],
								additionalProperties: false,
							},
							duplicate_policy: {
				...
				spawnPlan: {
					...
					required: ["whyParallel", "whyNotLocal", "independence", "expectedReceiptShape", "maxInlineTokens"],
					additionalProperties: false,
				...
				ops: {
					...
					items: {
						...
						properties: {
							op: {
								type: "string",
								enum: ["init", "start", "done", "rm", "drop", "append", "note"],
	...
	telegram_send: {
		...
		label: "TelegramSend",
		description:
			"Send a file from the current workspace to the connected Telegram chat. Recognized images are converted to Telegram-compatible photos when possible, including WebP; other files are sent as documents with their MIME type preserved. The path must resolve (after following symlinks) to a regular file inside the project root; paths outside the workspace are rejected.",
		parameters: {
			type: "object",
			properties: {
				path: {
					type: "string",
					description:
						"file path (absolute or relative to cwd) to send to Telegram; must resolve inside the workspace",
				},
				caption: {
