...
test("ACP caps cancelled provider calls that never settle", async () => {
	const sdk = new FakeSdkClient();
	...
	try {
		for (let index = 0; index < 64; index++) {
			...
			sdk.emit({ type: "reverse_cancel", id });
		}
		await Bun.sleep(0);
		...
		await waitFor(
			() => sdk.frames.some(frame => frame.type === "reverse_response" && frame.id === "capacity-rejected"),
			"reverse provider capacity response",
		);
		expect(
			sdk.frames.find(frame => frame.type === "reverse_response" && frame.id === "capacity-rejected"),
		).toMatchObject({ ok: false, error: { code: "acp_reverse_capacity" } });
		expect(calls).toBe(64);
	} finally {
		pending.resolve({ selected: "late" });
		await adapter.close();
	}
});

test("ACP reverse cancellation kills and releases a terminal created after cancellation", async () => {
	const sdk = new FakeSdkClient();
	...
	try {
		sdk.emit({
			type: "reverse_request",
			id: "late-terminal-create",
			connectionId: sdk.connectionId,
			capability: "terminal",
			leaseId: "lease-1",
			payload: { method: "terminal.create", payload: { command: "echo late" } },
		});
		await waitFor(() => cancellationSignal !== undefined, "terminal create cancellation signal");
		sdk.emit({ type: "reverse_cancel", id: "late-terminal-create" });
		expect(cancellationSignal?.aborted).toBe(true);
		created.resolve({ terminalId: "term-late" });
		await waitFor(() => calls.includes("terminal.release"), "late terminal cleanup");
		expect(calls).toEqual(["terminal.create", "terminal.kill", "terminal.release"]);
		expect(sdk.frames.some(frame => frame.type === "reverse_response" && frame.id === "late-terminal-create")).toBe(
			false,
		);
	} finally {
		await adapter.close();
