...
describe("SessionSdkHost", () => {
	...
	test("bounds async frame work per connection and fences only the saturated producer", async () => {
		let receive!: (connectionId: string, frame: Record<string, unknown>) => void;
		const gates = Array.from({ length: 32 }, () => Promise.withResolvers<void>());
		let started = 0;
		const overflowed: string[] = [];
		let host!: SessionSdkHost;
		host = new SessionSdkHost({
			sessionId: "bounded-frames",
			stateRoot: "/tmp/bounded-frames",
			token: "token",
			sendFrame: () => {},
			onFrame: handler => {
				receive = handler;
			...
			control: async () => {
				...
				await gate?.promise;
				return { id: "done", ok: true };
			},
			onFrameOverflow: connectionId => {
				overflowed.push(connectionId);
				host.handleDisconnect(connectionId);
			},
		});
		await host.start();
		for (let index = 0; index < 32; index++) {
			receive("producer", { type: "control_request", id: `r${index}`, operation: "test", input: {} });
		}
		await Bun.sleep(0);
		expect(started).toBe(32);
		receive("producer", { type: "control_request", id: "overflow", operation: "test", input: {} });
		await Bun.sleep(0);
		expect(overflowed).toEqual(["producer"]);
		expect(host.isConnectionOpen("producer")).toBe(false);
		for (const gate of gates) gate.resolve();
		await host.stop();
	});

	test("quarantines only the exact live provider lease identity", async () => {
		let receive!: (connectionId: string, frame: Record<string, unknown>) => void;
		const sent: Array<Record<string, unknown>> = [];
		const host = new SessionSdkHost({
			sessionId: "lease-quarantine",
			stateRoot: "/tmp/lease-quarantine",
			token: "token",
			sendFrame: (_connectionId, frame) => {
				sent.push(frame);
			},
		...
		await Bun.sleep(0);
		const leaseId = String(sent.find(frame => frame.type === "register_provider_result")?.leaseId);
		const generation = host.connectionGeneration("owner");
		expect(
			host.quarantineProviderLease({
				leaseId,
				connectionId: "owner",
		...
		expect(host.reverse.getLease("terminal")?.active).toBe(true);
		expect(
			host.quarantineProviderLease({ leaseId, connectionId: "owner", connectionGeneration: generation, fence: "f" }),
		).toBe(true);
		expect(host.reverse.getLease("terminal")).toBeUndefined();
		await host.stop();
	});
});
