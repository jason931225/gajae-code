...
beforeEach(() => {
	tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-streaming-edit-"));
});
...
it("isolates concurrent session auth stores in OS-owned temporary directories", async () => {
	const roots = Array.from({ length: 8 }, () =>
		fs.mkdtempSync(path.join(os.tmpdir(), "pi-streaming-edit-concurrent-")),
	);
	const created = await Promise.all(
		roots.map(root => createSession(root, createStreamForDiff("sample.txt", [], {}), editTool)),
	);
	try {
		expect(new Set(roots).size).toBe(roots.length);
		for (const root of roots) expect(fs.existsSync(path.join(root, "testauth.db"))).toBe(true);
	} finally {
		await Promise.all(
			created.map(async ({ session, authStorage }) => {
				try {
