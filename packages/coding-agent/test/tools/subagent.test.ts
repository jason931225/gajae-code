...
import { SubagentTool, capCodePointsAndBytes, type ToolSession } from "../../src/tools";
...
async function inspectCompletedSubagent(
	...
	const result = await tool.execute(`subagent-${verbosity}`, { action: "inspect", ids: [`0-${verbosity}`], verbosity });
...
describe("subagent preview byte caps", () => {
	it("keeps cap-1 and exact multibyte payloads unchanged and caps cap+1 payloads per tier", () => {
		for (const tier of PREVIEW_TIERS) {
			...
			expect([...capped].filter(codePoint => codePoint === "…"), tier.name).toHaveLength(1);
		...
		for (const character of ["\u200b", "\u0301"]) {
			for (const tier of PREVIEW_TIERS) {
				...
				for (const byteBudget of [tier.bytes - 1, tier.bytes, tier.bytes + 1]) {
					...
					expect(Buffer.byteLength(capped), `${character} ${tier.name} ${byteBudget}`).toBeLessThanOrEqual(byteBudget);
					expect(capped, `${character} ${tier.name} ${byteBudget}`).toEndWith("…");
					expect([...capped].filter(codePoint => codePoint === "…"), `${character} ${tier.name} ${byteBudget}`).toHaveLength(1);
				}
				...
				expect([...preview].filter(codePoint => codePoint === "…"), `${character} ${tier.name} preview`).toHaveLength(1);
				...
				expect([...snapshot.label].filter(codePoint => codePoint === "…"), `${character} ${tier.name} label`).toHaveLength(1);
				if (tier.verbosity === "full") {
					...
					expect([...assignment].filter(codePoint => codePoint === "…"), `${character} full assignment`).toHaveLength(1);
				}
			}
		}
	});
