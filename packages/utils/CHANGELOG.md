...
- A `GJC_CODING_AGENT_DIR` planted by the caller's project `.env` no longer selects the agent directory. Bun loads `cwd/.env` into `process.env` before any module runs, so a repository could point the agent directory at one it ships and have that directory's `.env` treated as a trusted credential source — recovering every endpoint and credential redirect the trust boundary rejects. The override is now ignored when it matches what the project `.env` sets; an operator setting it from their shell is unaffected. The `.env` parsing primitives moved to a leaf `env-file` module so `dirs` can use them without a cycle; their public surface is unchanged.
